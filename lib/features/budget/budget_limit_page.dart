...
class _BudgetLimitPageState extends State<BudgetLimitPage> {
  ...
  int _limit = 300000; // 기본값

  // ---- 월 키 유틸 (두 페이지에서 동일 규칙 사용) ----
  String _ym([DateTime? d]) {
    final n = d ?? DateTime.now();
    return '${n.year}${n.month.toString().padLeft(2, '0')}';
  }
  String get _limitKey => 'budget_limit_${_ym()}';
  ...
  Future<void> _loadLimit() async {
    ...
    final v = sp.getInt(_limitKey);
    if (!mounted) return;
    setState(() => _limit = v ?? 2000000);
  }
      ...
      builder: (ctx) {
        ...
        return Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            const Text('예산 한도 수정', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)),
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              for (final p in presets)
                OutlinedButton(onPressed: () { c.text = _won.format(_parse(c.text) + p); fmt(); }, child: Text('${_won.format(p)}원')),
              TextButton(
                onPressed: () { c.text = '0'; fmt(); FocusScope.of(ctx).requestFocus(focus); },
                child: const Text('초기화'),
              ),
            ]),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: OutlinedButton(onPressed: () => Navigator.pop(ctx), child: const Text('취소'))),
              ...
              Expanded(
                child: FilledButton(
                  onPressed: () {
                    ...
                    if (v <= 0) {
                      ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('1원 이상 입력해 주세요.')));
                      return;
                    }
                    Navigator.pop(ctx, v);
                  },
      ...
      bottomNavigationBar: Padding(
        ...
        child: FilledButton.icon(
          style: FilledButton.styleFrom(
            backgroundColor: navy, padding: const EdgeInsets.symmetric(vertical: 14),
          ...
          onPressed: () async {
            ...
            if (changed != null && changed > 0) {
              final sp = await SharedPreferences.getInstance();
              await sp.setInt(_limitKey, changed);
            ...
            child: Row(children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('이번 달 예산 한도', style: TextStyle(color: Colors.white70, fontSize: 12)),
